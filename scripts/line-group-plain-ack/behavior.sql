...
END $$;
-- A queue failure must not retain message text or acknowledge an unqueued event.
ALTER TABLE public.line_oa_outbound_messages ADD CONSTRAINT reject_test_queue CHECK (target_id <> 'contract-active') NOT VALID;
DO $$
DECLARE result record; rejected boolean := false;
BEGIN
 BEGIN
  SELECT * INTO result FROM public.rpc_ingest_line_webhook(
   '{"events":[{"webhookEventId":"contract-failure","type":"message","source":{"groupId":"contract-active","userId":"fixture-user"},"message":{"type":"text","text":"must never persist"}}]}', 'fixture', 'fixture');
 EXCEPTION WHEN SQLSTATE 'P0001' THEN rejected := true;
 END;
 IF EXISTS (SELECT 1 FROM public.line_oa_inbound_messages WHERE webhook_event_id='contract-failure') THEN
  RAISE EXCEPTION 'FAIL: failed queue persisted an inbound receipt and private content';
 END IF;
 IF NOT rejected THEN RAISE EXCEPTION 'FAIL: failed queue was accepted instead of retryable'; END IF;
 RAISE NOTICE 'PASS: queue failure rolls back without receipt/content';
END $$;
ROLLBACK;
