...
2. THE bot SHALL เข้าทุกกลุ่ม — กลุ่ม internal: **ช่างส่งรูปเฉย ๆ → bot เสนอปุ่ม ("รูปงานเสร็จ / แจ้งปัญหา")** ไม่ต้องจำ command (hashtag `#ปัญหา`/`#ผูก` เป็น shortcut ทางเลือกของ power user) → capture `installation_proof`/issue; กลุ่ม customer: **เฉพาะ template ที่ `audience='customer'`** — enforce ที่ DB CHECK + Edge Function (Guardrail — ส่งผิดกลุ่ม = error)
2a. **[UX tenet]** ผู้ใช้หน้างาน SHALL ไม่เห็น id/key/ศัพท์โครงสร้างใด ๆ (room_id, work_item_id, template_key, capture_type) — เห็นแค่ชื่อภาษาคน ("ครัว", "งานของคุณ"); หลังบ้าน **infer ห้อง/เลน/โปรเจกต์จาก assignment + group binding ก่อนถาม** ถามเฉพาะเมื่อกำกวมจริงและถามด้วยปุ่ม; การผูกกลุ่มใช้ปุ่ม ("นี่กลุ่มทีม / นี่กลุ่มลูกค้า") + deep-link ไม่ให้พิมพ์รหัส (ดู `ux-tenet-audit-v1.md`)
3. THE พนักงาน SHALL ผูกตัวตน LINE↔บัญชี **ครั้งเดียวผ่านลิงก์ + LINE Login** (**`identity_binding` ตารางเดียว — ADR-038 ยุบ `line_staff_identity`**: ขยาย +consent_at/bound_at/revoked_at, UNIQUE ต่อ line_user_id) — รูป/ข้อความจาก LINE ต้อง resolve เป็นพนักงานตัวจริงเพื่อ audit "ใครทำอะไรที่ไหนเมื่อไหร่"
