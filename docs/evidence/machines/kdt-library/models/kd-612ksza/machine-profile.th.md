# โปรไฟล์เครื่องจักร (Machine Profile) — KDT KD-612KSZA
...
|---|---|---|---|
| พื้นที่ทำงาน / ระยะวิ่ง (`envelope`) | Field labels present but no numeric values on KDT Iberica page (board length/width/thickness listed without figures) | ยืนยันจากเอกสาร (Verified in documents) | [kdtiberica.com](https://kdtiberica.com/en/drills/cnc-drills/cnc-drill-kd-612khsza/) |
| ความสามารถ (`capabilities`) | Double upper drilling unit + upper milling + upper saw unit + lower drilling + lower milling (labels only, values not shown) | ยืนยันจากเอกสาร (Verified in documents) | [kdtiberica.com](https://kdtiberica.com/en/drills/cnc-drills/cnc-drill-kd-612khsza/) |
...
| ระบบอัตโนมัติ (`automation`) | Smart switch tool (KDT tagline) | ยืนยันจากเอกสาร (Verified in documents) | [en.kdtmac.com](https://en.kdtmac.com/Exhibitions_product_detail_1/148.html) |
| ขนาด / กำลังไฟ / น้ำหนัก (`footprint_power_weight`) | Unknown | ยังไม่ทราบ (Unknown) | ไม่มี |

## 4. ข้อขัดแย้งที่บันทึกไว้ (ไม่ตัดสินให้)

**ข้อขัดแย้ง (Conflict):** KDT-official exhibition page 148 is image-only (no OCR-legible table). Closest documented sibling is KD-612KSZ (Leda quotation PDF): Top 26V+12H, Bottom 9V; spindle Top 5.5kW/Bottom 3.5kW; 140/90/50 m/min; overall 6040x3134x2190; total 28.22kW; 5 ATC types. KSZA specifics remain Unknown.
