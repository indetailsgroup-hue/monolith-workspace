# โปรไฟล์เครื่องจักร (Machine Profile) — KDT KB-399R
...
| รุ่น | **KB-399R** |
| ชื่อเรียกอื่น (aliases) | KE-399R |
| ตระกูล | Banding Machine (Edge Bander) |
| สถานะในแค็ตตาล็อก | current-global |
| ภูมิภาค | Global |
| วัตถุประสงค์การใช้งาน | Edge bander (tilt/straight) |
| แหล่งที่มา (URLs) | [en.kdtmac.com](https://en.kdtmac.com/products_detail/212.html) |
...
|---|---|---|---|
| พื้นที่ทำงาน / ระยะวิ่ง (`envelope`) | Panel T 9-25/9-22/9-20 (tilt) or 9-60 (straight); L >=300(tilt)/>=120(straight); W >=150(tilt)/>=90(straight) | ยืนยันจากเอกสาร (Verified in documents) | [en.kdtmac.com](https://en.kdtmac.com/products_detail/212.html) |
| ความสามารถ (`capabilities`) | Beveled/tilt edge bander (=KE-399R); tape 0.5-0.8(tilt)/0.4-3(straight)mm | ยืนยันจากเอกสาร (Verified in documents) | [en.kdtmac.com](https://en.kdtmac.com/products_detail/212.html) |
| กำลัง / ความจุ (`capacities`) | Feeding 10-15-18-24(tilt), 10(straight) m/min | ยืนยันจากเอกสาร (Verified in documents) | [en.kdtmac.com](https://en.kdtmac.com/products_detail/212.html) |
...
| ขนาด / กำลังไฟ / น้ำหนัก (`footprint_power_weight`) | Total power 42.74kW; overall 10291x990x1730mm; dust Ø125*10+Ø50*1; weight Unknown | ยืนยันจากเอกสาร (Verified in documents) | [en.kdtmac.com](https://en.kdtmac.com/products_detail/212.html) |

## 4. ข้อขัดแย้งที่บันทึกไว้ (ไม่ตัดสินให้)

**ข้อขัดแย้ง (Conflict):** Listed as KB-399R but official spec table header reads KE-399R (same machine, dual code).
