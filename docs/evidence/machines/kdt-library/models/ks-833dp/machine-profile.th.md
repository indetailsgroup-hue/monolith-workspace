# โปรไฟล์เครื่องจักร (Machine Profile) — KDT KS-833DP

> `profile_id: kdt.saw.ks_833dp@0.1-research-draft` · as-of: 2026-07-17
> [EN edition](machine-profile.en.md)

**Saw Tech** — Beam saw; compatible with various order-splitting software
...
| ภูมิภาค | Global |
| วัตถุประสงค์การใช้งาน | Beam saw; compatible with various order-splitting software |
| แหล่งที่มา (URLs) | [en.kdtmac.com](https://en.kdtmac.com/products_detail/217.html) |
...
|---|---|---|---|
| พื้นที่ทำงาน / ระยะวิ่ง (`envelope`) | Front feeding ~3330 x rear ~3000; KDT-official page 217 (products_detail) is image-only - values not OCR-legible | ยืนยันจากเอกสาร (Verified in documents) | [en.kdtmac.com](https://en.kdtmac.com/products_detail/217.html) |
...
| ระบบอัตโนมัติ (`automation`) | Dual-pusher (DP) rear-loading beam saw | ยืนยันจากเอกสาร (Verified in documents) | [en.kdtmac.com](https://en.kdtmac.com/products_detail/217.html) |
| ขนาด / กำลังไฟ / น้ำหนัก (`footprint_power_weight`) | Unknown | ยังไม่ทราบ (Unknown) | ไม่มี |

## 4. ข้อขัดแย้งที่บันทึกไว้ (ไม่ตัดสินให้)

**ข้อขัดแย้ง (Conflict):** KDT-official detail page 217 has image-only specs (not OCR-legible). No legible official numeric table located; values remain Unknown per grounding rule.
