# โปรไฟล์เครื่องจักร (Machine Profile) — KDT KS-543HLS
...
| รุ่น | **KS-543HLS** |
| ชื่อเรียกอื่น (aliases) | KS-543HL, KS-543H |
| ตระกูล | Saw Tech |
| สถานะในแค็ตตาล็อก | current-global |
| ภูมิภาค | Global |
| วัตถุประสงค์การใช้งาน | Panel saw; double push handle (twin pusher) |
...
|---|---|---|---|
| พื้นที่ทำงาน / ระยะวิ่ง (`envelope`) | Front feeding 4280x4280x120 / rear feeding 4000x2200 (KDT distributor kdtmac.lt for HLM sibling); max cut thickness 120mm (saw extension 140) | ยืนยันจากเอกสาร (Verified in documents) | [kdtmac.lt](https://kdtmac.lt/cnc-panel-saw-beam-saw-ks-542hl) |
| ความสามารถ (`capabilities`) | Twin-pusher / double push handle; Maglev + linear-motor saw carriage; main saw 22kW, scoring 2.2kW; blade 450/75 + 200/50 (kdtmac.lt KS-543HL/HLM sibling data) | ยืนยันจากเอกสาร (Verified in documents) | [kdtmac.lt](https://kdtmac.lt/cnc-panel-saw-beam-saw-ks-542hl) |
| กำลัง / ความจุ (`capacities`) | Saw car moving speed 200 m/min (KDT listing tagline); carriage travel 160 / idle 200 / pusher 95 m/min (kdtmac.lt sibling) | ยืนยันจากเอกสาร (Verified in documents) | [en.kdtmac.com](https://en.kdtmac.com/products_list/6.html) |
...
| ระบบอัตโนมัติ (`automation`) | Intelligent simultaneous feeding/processing/feeding; twin pusher + 2 movable group grippers; hydraulic table 4000kg (kdtmac.lt sibling) | ยืนยันจากเอกสาร (Verified in documents) | [en.kdtmac.com](https://en.kdtmac.com/products_list/6.html) |
| ขนาด / กำลังไฟ / น้ำหนัก (`footprint_power_weight`) | Installed power 51kW; overall 12970x7570x2030mm; weight 11000kg (kdtmac.lt KS-543HLM sibling) | ยืนยันจากเอกสาร (Verified in documents) | [kdtmac.lt](https://kdtmac.lt/cnc-panel-saw-beam-saw-ks-542hl) |

## 4. ข้อขัดแย้งที่บันทึกไว้ (ไม่ตัดสินให้)

**ข้อขัดแย้ง (Conflict):** KDT-official exhibition page 1 is image-only (no legible spec table). Distributor kdtmac.lt page is for KS-542HL/543HLM variants; exact KS-543HLS values not confirmed on an official page. Values marked from sibling variant.
