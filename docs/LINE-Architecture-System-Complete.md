...
| **พนักงาน** (ช่าง/หัวหน้า/sale/บัญชี) | LINE user_id ↔ auth user | `identity_binding` (ขยาย — ADR-038) 📝 | **deterministic** (ลิงก์ + LINE Login + consent, ครั้งเดียว) — เพื่อ audit "ใครทำ" |
...
### 4.4 พนักงานออฟฟิศ — อนุมัติ work item ผ่าน LINE 🔵 (โค้ดครบ — รอ ops)
Office staff (ขาย/วัด/วางแผน) เห็นสถานะงานใน canonical process + กดอนุมัติ/ส่งต่อขั้นจาก LINE · **Phase 13 ปิดแล้ว (2026-07-06)** — เหลือ ops ตาม `docs/OPS-RUNBOOK-Wave2.md` (deploy + cron + seeds)
...
```sql
identity_binding += consent_at, bound_at, revoked_at   -- ADR-038: ขยายตารางเดิมของ workflow (app_role มีจาก 0084) ไม่สร้าง line_staff_identity
...
| Workflow work-item approval ผ่าน LINE | 🔵 | **Phase 13 ปิดแล้ว (2026-07-06)** — delivery chain จริงครบ (0081 + worker push); เหลือ ops: cron registration + deploy |
...
| **Field Purchase** (ซื้อของหน้างาน: ขออนุมัติ→ซื้อ→เบิกคืน) | 📝 | `.kiro/specs/field-purchase/` — front-first (ช่างเห็น 2 การกระทำ); reuse expense_document ✅ + approval-postback ✅; net-new = capture `field_purchase_request` + approval routing ตามวงเงิน |

---

## 9. Dependencies (สำคัญก่อน implement)

1. ~~workflow Phase 13~~ → **ปิดแล้ว (2026-07-06)**: delivery chain จริงครบ — `rpc_claim_pending_notifications` v2 (0081: resolve ผู้รับจาก identity_binding + render template + Vault token ref) → `notification-retry-worker` ส่ง LINE push ตรง → record/backoff ถูกสูตร (แก้บั๊กเดิมที่ mark 'sent' โดยไม่เคยส่งจริง) — เหลือ **ops: ลงทะเบียน cron (pg_cron → worker ทุก 1 นาที) + deploy** ก่อนเปิดใช้จริง
2. Installation groups พึ่ง `identity_binding` ที่ขยาย (ADR-038) + group tables (net-new) — landing พร้อม installation-pm migrations (Phase 1.1)
