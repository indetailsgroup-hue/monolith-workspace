...
> ### 🧭 หลักการออกแบบเหนือทุกอย่าง (Governing UX Tenet) — "หน้าบ้านง่ายจนคนรัก หลังบ้านรับความยากไว้เอง"
> **ทุกคนต้องรักระบบนี้ และทุกบทบาทต้องใช้ง่าย** — ความเข้มงวด/ความซับซ้อน (governance, gate, RACI, audit, fraud check, RLS, job cost) **ต้องอยู่หลังบ้านทั้งหมด** ผู้ใช้หน้างานไม่ควรเห็น/ไม่ต้องเรียนรู้ศัพท์ระบบใด ๆ
> - **เกณฑ์วัด:** ช่าง/ลูกค้าทำงานให้จบได้ด้วยการกระทำน้อยที่สุด (ถ่ายรูป + พิมพ์บรรทัดเดียว + กดปุ่มเดียว) — ไม่มีฟอร์มยาว, ไม่ต้องเลือก project/vendor code, ไม่ต้องรู้ว่าใครอนุมัติ
> - **ราคาที่ต้องจ่าย = หลังบ้านต้องฉลาดและรอบคอบขึ้น:** ยิ่งซ่อนจากผู้ใช้มาก back-end ยิ่งต้อง infer/route/verify ให้แม่น (ผูก work item จาก group binding อัตโนมัติ, route อนุมัติตามวงเงินเอง, match ใบเสร็จกับคำขอเอง) — นี่คือส่วนที่ทีมต้อง**คิดให้รอบคอบที่สุด**
> - ใช้ทดสอบทุกฟีเจอร์: *"ผู้ใช้ที่ไม่เคยอบรมเลย ทำสิ่งนี้จบได้ในกี่ tap? เห็นศัพท์ระบบกี่คำ?"* — ถ้าคำตอบไม่ใช่ "น้อยที่สุด" แปลว่าเราผลักความยากไปให้ผู้ใช้ผิดที่
> - **ผล audit ทั้งระบบ:** `docs/UX-Tenet-System-Audit-v1.md` (ทุก persona + กติกาถาวร S1–S6) และ `.kiro/specs/installation-pm/ux-tenet-audit-v1.md` (งานติดตั้ง — แก้ 9 จุด)
...
**[P0] Notifications ✅ (Phase 13 ปิด 2026-07-06 — delivery chain ครบ: claim+resolve (0081: identity_binding + template render + Vault ref) → LINE push จริง → record/backoff (0019); เหลือ ops: ลงทะเบียน cron + deploy)**
...
- ✅ delivery จริงครบ (2026-07-06): `rpc_claim_pending_notifications` v2 (0081) resolve ผู้รับ+render+token ref → worker push LINE → backoff ถูกสูตร — เหลือ ops: cron registration
...
- `.kiro/specs/*/` — requirements.md / design.md / tasks.md ต่อ spec — รวม spec ใหม่จาก session นี้: **`curved-panel-system/`** (พร้อม kerf patch archive ที่ติดป้าย SUPERSEDED), **`entitlement-tier/`** (spec + `schema-draft-v0.3.sql` + `tests-negative.sql` + tier-matrix/schema-design v0.3 + `research-oneclickcabinet-v4.1.md` + v0.4 delta Site PM), **`installation-pm/`** (field PM แนว KANNA — requirements/design/tasks + line-architecture + form templates + **ux-tenet-audit** + external draft archive) และ **`field-purchase/`** (ซื้อของหน้างาน front-first — ปิดช่องว่างขออนุมัติก่อนซื้อ)
- **`docs/DAPH-Organization-and-JD-Complete.md`** (+`.html`) — **ฉบับสมบูรณ์รวมทุกตำแหน่ง 38 รายการ** (โครงสร้างองค์กร + 27 JD ทางการ + 11 ร่าง) = SSOT เรื่องตำแหน่งงาน/สายบังคับบัญชา/อำนาจอนุมัติ ใช้กับ workflow RACI + IAM roles + Installation PM; ที่มายังเก็บแยกไว้ที่ `DAPH-org-structure-from-JD-2025.md` + `JD-drafts-missing-positions-2026-07.md`
