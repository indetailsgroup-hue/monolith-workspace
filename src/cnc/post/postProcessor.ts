 ...
 *
 * @version 1.4.0 - Phase T028-P4: Added MPR, CIX, XXL post-processors
 */
...
const postProcessorRegistry: Record<GcodeDialect, PostProcessor> = {
  ...
  MPR: mprPostProcessor,               // Homag/WoodWOP native MPR format
  CIX: cixPostProcessor,               // Biesse CIX XML format
  XXL: xxlPostProcessor,               // SCM/Morbidelli Xilog XXL format
};
...
const machineDialectMap: Record<string, GcodeDialect> = {
  KDT: 'FANUC',
  BIESSE: 'CIX',           // Native CIX XML format (was BIESSE_ISO G-code)
  BIESSE_ISO: 'BIESSE_ISO', // G-code fallback for Biesse machines
  HOMAG: 'MPR',            // Native WoodWOP format (was WEEKE G-code)
  HOMAG_ISO: 'WEEKE',     // G-code fallback for Homag machines
  SCM: 'XXL',              // Native Xilog XXL format (was FANUC G-code)
  SCM_ISO: 'FANUC',       // G-code fallback for SCM machines
  GENERIC: 'FANUC',
};

/**
 * Map dialect aliases to canonical dialect names.
 * Handles variations like machine.dialect='BIESSE' -> registry key 'CIX'.
 */
const dialectAliases: Record<string, GcodeDialect> = {
  BIESSE: 'CIX',          // Biesse machines default to CIX XML format
  FANUC: 'FANUC',
  BIESSE_ISO: 'BIESSE_ISO',
  MPR: 'MPR',
  WEEKE: 'WEEKE',
  CIX: 'CIX',
  XXL: 'XXL',             // SCM machines default to Xilog XXL format
};
