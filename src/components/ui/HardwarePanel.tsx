...
import { Package, ShoppingCart, Wrench, Crosshair } from 'lucide-react';
...
type HardwareTab = 'library' | 'catalog' | 'connectors' | 'drilling';
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex border-b border-[#333] shrink-0">
        ...
        <button
          ...
          className={`flex-1 px-2 py-1.5 text-[10px] font-medium transition-all flex items-center justify-center gap-1.5 relative ${
            activeTab === 'drilling' ? 'text-cyan-400 bg-surface-2' : 'text-gray-500 hover:text-white hover:bg-surface-2/50'
          }`}
        >
          {activeTab === 'drilling' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-cyan-400" />}
          <Crosshair size={12} />
          <span>Drilling</span>
        </button>
      </div>
      ...
      <div className="flex-1 overflow-hidden">
        ...
        {activeTab === 'drilling' && <DrillingPatternView />}
      </div>
    </div>
  );
}
