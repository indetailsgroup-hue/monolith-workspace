...
// Preview-only keys must never leak into manufacturing/drill-map generation.
// Keep this list in sync with the UI-only transform fields on MinifixFullConfig.
export const MINIFIX_PREVIEW_ONLY_KEYS = [
  'flipVertical',
  'flipHorizontal',
  'rotationX',
  'rotationY',
...
export type MinifixPreviewOnlyKey = (typeof MINIFIX_PREVIEW_ONLY_KEYS)[number];

export function sanitizeManufacturingConfig<T extends Record<string, unknown>>(config: T): Omit<T, MinifixPreviewOnlyKey> {
  const copy = { ...config } as Record<string, unknown>;
  for (const key of MINIFIX_PREVIEW_ONLY_KEYS) {
    delete copy[key];
  }
  return copy as Omit<T, MinifixPreviewOnlyKey>;
}

export function assertNoPreviewKeys(cfg: Record<string, unknown>, context = 'MinifixConfig'): void {
  const found = MINIFIX_PREVIEW_ONLY_KEYS.filter((key) => key in cfg);
  if (found.length > 0) {
    throw new Error(`[${context}] Preview-only keys leaked into manufacturing config: ${found.join(', ')}`);
  }
}
...
export const DEFAULT_MINIFIX_CONFIG: MinifixFullConfig = {
  ...
  // Dowel
  includeDowel: true,      // Dowels enabled per baseline spec (Ø8×30, System 32 pitch)
  dowelDia: 8,
  dowelLength: 30,
  dowelOffset: 32,         // System 32 pitch (was incorrectly 6)
