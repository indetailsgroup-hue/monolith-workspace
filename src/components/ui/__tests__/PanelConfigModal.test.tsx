...
describe('PanelConfigModal', () => {
  ...
  describe('rendering', () => {
    ...
    it('should render "Individual Configuration" subtitle', () => {
      ...
      expect(screen.getByText('Individual Configuration')).toBeInTheDocument();
    });
  ...
  describe('position controls (shelves/dividers)', () => {
    it('should show position controls for SHELF panels', () => {
      ...
      expect(screen.getByText('Back Setback (LED)')).toBeInTheDocument();
    });

    it('should NOT show position controls for SIDE panels', () => {
      render(<PanelConfigModal panelId="panel-left-1" onClose={mockOnClose} />);

      expect(screen.queryByText('Front Setback')).not.toBeInTheDocument();
      expect(screen.queryByText('Back Setback (LED)')).not.toBeInTheDocument();
    });

    it('should show Gap Height control for shelves', () => {
      render(<PanelConfigModal panelId="panel-shelf-1" onClose={mockOnClose} />);

      expect(screen.getByText(/Gap Height/)).toBeInTheDocument();
    });
  ...
  describe('core material selection', () => {
    it('should render Core Structure section', () => {
      ...
      expect(screen.getByText('Core Structure')).toBeInTheDocument();
    });

    it('should list all core materials', () => {
      render(<PanelConfigModal panelId="panel-shelf-1" onClose={mockOnClose} />);

      expect(screen.getByText('Particle Board 16mm')).toBeInTheDocument();
      expect(screen.getByText('MDF 18mm')).toBeInTheDocument();
    });

    it('should call updatePanelMaterial when core is changed', () => {
      render(<PanelConfigModal panelId="panel-shelf-1" onClose={mockOnClose} />);

      const mdfButton = screen.getByText('MDF 18mm');
      fireEvent.click(mdfButton);
  ...
  describe('face material selection', () => {
    it('should render Face A section', () => {
      ...
      expect(screen.getByText('Face A (Primary)')).toBeInTheDocument();
    });

    it('should render Face B section', () => {
      render(<PanelConfigModal panelId="panel-shelf-1" onClose={mockOnClose} />);

      expect(screen.getByText('Face B (Outer)')).toBeInTheDocument();
    });
  ...
  describe('manufacturing data', () => {
    ...
    it('should show FINISH dimensions', () => {
      ...
      expect(screen.getByText('800.0 × 500.0')).toBeInTheDocument();
    });
    ...
    it('should show CUT dimensions', () => {
      ...
      expect(screen.getByText('799.0 × 499.0')).toBeInTheDocument();
    });

    it('should show thickness', () => {
      render(<PanelConfigModal panelId="panel-shelf-1" onClose={mockOnClose} />);

      expect(screen.getByText('Thk: 16.6mm')).toBeInTheDocument();
    });
    ...
    it('should show formula for shelves', () => {
      ...
      expect(screen.getByText(/D\(550\)/)).toBeInTheDocument();
    });
  });
