-- Migration: capture_promote_adr_updates — capture-spine (ADR-029/031/033 ที่จุด promote — replace ครั้งเดียว)
-- Depends on: 0074 (rpc_capture_promote 5 branches), 0076 (normalize_name), 0077 (name_normalized/needs_review), 0078 (corrected_fields)
--
-- เปลี่ยน 3 จุดจาก 0074 (ที่เหลือคงเดิมทุกบรรทัด — reuse-not-fork):
--   [ADR-033] effective payload = ai_payload overlay ด้วย corrected_fields (ค่าที่มนุษย์แก้ต้องถูก commit ไม่ใช่ค่า AI ที่ผิด)
--   [ADR-029] costing branch: จับคู่/derive code ผ่าน name_normalized + auto-สร้างติด needs_review=true
--   [ADR-031] spec branch: ผู้ promote ต้องเป็น RACI Designer accountable (จาก knowledge_import current) หรือ governance
--             — "Designer Lead" ไม่ใช่ C12 app role; ใช้แหล่งเดียวกับ approver-resolver (0014) ไม่ hardcode role
--             — accountable ว่าง/ไม่มี → fail-safe block (Req 3.4 pattern)
