-- Migration: capture_survey_adapter — capture-spine (L3 commit-target adapter #5 — 'SiteSurveyZone')
-- Depends on: 0072 (rpc_capture_promote + 4 branches), 0073 (site_survey_zone), C12
--
-- L3 adapter #5: promote ของ site_survey (commit_target='SiteSurveyZone') → บันทึกข้อมูลวัดหน้างานต่อ zone.
--   fail-safe: site_code/zone/dimension/mep ต้องครบ. versioning ต่อ (site,zone): re-survey → supersede+v+1.
-- รักษา branch Work_Item/ledger/costing/Released_Spec ครบ — reuse-not-fork. ปิด L3 ครบ 5 seam ตาม 0062.
