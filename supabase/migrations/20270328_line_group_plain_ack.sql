  ...
  for v_event in select jsonb_array_elements(v_events) loop
    ...
    if v_group_id is not null then
      ...
      begin
        ...
        -- The baseline handler returns errors as text. Never persist the event
        -- or acknowledge it as processed when staging failed. Roll back the
        -- transaction so LINE can retry; do not expose SQLERRM/message content.
        if v_group_result like 'handler_error:%' then
          raise exception using errcode = 'P0001', message = 'group_event_processing_failed';
        end if;
