...
4. authenticated client bypass route policy ด้วยการ select ตาราง Factory job/event ตรง ๆ ไม่ได้
5. server actor context ที่ผิดรูปถูกปฏิเสธก่อนแก้ state หรือ audit
6. role ผิดประเภทถูกปฏิเสธสำหรับการบันทึก packet และ verification
7. transition ผิดลำดับคืน error และไม่เขียน event
8. งานสถานะ FROZEN บันทึก/อัปโหลด packet ไม่ได้, exportable ไม่ได้ และบันทึก verification ไม่ได้
9. actor name ที่หน้าตาเหมือน email ถูก ignore โดย `actor_name` ที่ persist ต้องเท่ากับ verified actor subject ID
10. positive control `DRAFT -> FROZEN -> RELEASED -> record packet -> verify` ผ่านและบันทึก server actor context
