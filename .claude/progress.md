...
- `src/culture-metrics/employeeSentimentStore.ts` — Zustand store with 6 actions: `fetchTimelineConfig`, `fetchSentimentSummary`, `fetchSentimentEntries`, `submitSentimentEntry` (plan-gate exempt), `createTimelineConfig`, `clearEstError`; supabase import from `../core/supabase`
- `src/culture-metrics/SentimentTimelineBoard.tsx` — PROFESSIONAL+ gated UI: plan-gate wall, loading, error banner, summary dimension cards with health badge + score, timeline entry rows, anonymous submit form (score input + dimension select); all `est-*` testids
- `src/culture-metrics/CultureDashboard.stories.tsx` — stories #13–16 appended: `AdminCreateSurveyFormVisible` (create-survey-form visible for admin), `AdminCreateSurveySubmit` (play: type title → submit → spy called), `AdminCreateSurveyBlankGuard` (play: click submit empty → spy NOT called), `AdminCreateMetricButton` (play: click create-metric-btn → spy called with CUSTOM payload); module-level `createEnpsSurveySpy`/`createMetricDefinitionSpy` added
- CHANGELOG: `[18.5.5]` entry added
